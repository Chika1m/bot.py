...
def check_signal():
    ...
    avg = sum(body(c) for c in prev) / len(prev)
    if avg == 0: return False
    return body(latest) / avg <= MAX_BODY_RATIO
                        ...
                        if t == "candles":
                            ...
                            if candles and candles[-1]["epoch"] == nc["epoch"]:
                                ...
                                if check_signal() and now - last_alert_time >= COOLDOWN_SECS:
                                    last_alert_time = now
                                    await send_telegram(session, "🚨 SELL — Boom 500 Index\n\n📉 Small green candle in downtrend\n⏱ Duration: 4 minutes\n🕐 " + datetime.utcnow().strftime("%H:%M UTC") + "\n\n⚠️ Trade at your own risk")
            except Exception as e:
